(function() {
	...
	var setPosition =  function(){
		if(mobileCheck()!=true){
				if(window.sessionStorage.getItem("chatCollapsed") != 'true'){
					...
					if(!window.sessionStorage.getItem("chatTopOffset")){
						$("#sp-chat-frame").css("top", $(window).height()-520);
						var chatTopOffset=$("#sp-chat-frame").css("top");
						if(chatTopOffset<10){
							chatTopOffset=10;
						}
						window.sessionStorage.setItem("chatTopOffset",chatTopOffset);
					} else {
						$("#sp-chat-frame").css("top", window.sessionStorage.getItem("chatTopOffset"));
					}
				} else {
					
					$('#sp-chat-frame', window.parent.document).css('top',$(window.parent).height()-49);
				}
				if(window.sessionStorage.getItem("chatLeftOffset")){
					$("#sp-chat-frame").css("left", window.sessionStorage.getItem("chatLeftOffset"));
					$("#sp-chat-fake").css("left", window.sessionStorage.getItem("chatLeftOffset"));
					
					
				} else{
					$("#sp-chat-frame").css("left", $(window.parent).width()-400);
				}
			} else{
				if(window.sessionStorage.getItem("chatCollapsed") == 'true'){
					$('#sp-chat-frame', window.parent.document).css('top',$(window.parent).height()-49);
				} else{
					window.sessionStorage.setItem("chatTopOffset",10);
        ...
        if(!isChatRendered()) {
            ...
            $("#sp-chat-frame").draggable({
                ...
                start: function( event, ui ) {
                    ...
                    $("#sp-chat-frame").addClass('drag');
                },
                drag: function(event, ui) {
                    var maxTop = Math.min($(window).height() - 249, ui.position.top);
                ...
                stop: function( event, ui ) {
					...
					window.sessionStorage.setItem("chatTopOffset",chatTopOffset);
					window.sessionStorage.setItem("chatLeftOffset", $('#sp-chat-frame').css('left'));
					$("#sp-chat-frame").removeClass('drag');
					$("#sp-chat-widget").offset({ left: $("#sp-chat-frame").offset().left+93});
					$("#sp-chat-widget").css('right','auto');
                }
            });
        }
    };
    ...
    var onInitialize = function() {
        ...
        $("body").append(
                ...
                "<div id=sp-drag-handle class=ui-draggable-handle></div>" +
                "<div id='sp-iframe-container'></div>" +
                "</div>" +
                "</div>");
				
		if(mobileCheck()) {
			$('#sp-root-container').addClass("mobile-version");
		} else {
			chatLeftOffset=parseInt(window.sessionStorage.getItem("chatLeftOffset"))+133;
			$("#sp-chat-widget").css("left", chatLeftOffset+'px');
		}
		...
		$(window).on("resize", function(event) {
			...
			if($(this).width()<parseInt(window.sessionStorage.getItem("chatLeftOffset"))+320){
				window.sessionStorage.removeItem("chatLeftOffset");
			}
			if($(this).height()<parseInt(window.sessionStorage.getItem("chatTopOffset"))+249 || parseInt(window.sessionStorage.getItem("chatTopOffset"))<0){
				window.sessionStorage.removeItem("chatTopOffset");
			}
			//alert(parseInt(window.sessionStorage.getItem("chatTopOffset"))+249);
			setPosition();
		})
